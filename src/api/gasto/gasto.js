...
router.get("/gasto/itinerario/:itinerarioId/user/:userId", async (req, res, next) => {
    ...
    try {
        ...
        const result = await Gasto.findOne({
          ...
          attributes: [[Sequelize.fn('COALESCE', Sequelize.fn('SUM', Sequelize.col('Gasto.monto')), 0), 'totalGastos']],
          raw: true,
        });
    ...
    //Calcula los gatos de todos los usuarios en un itinerario
...
router.delete("/gasto/:gastoId", async (req, res, next) => {
    ...
    res
      .status(200)
      .send(
        `Gasto with ID ${gastoId} was deleted successfully`
      );
